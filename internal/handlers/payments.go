...
<h2>1. Предмет</h2>
<p>Пользователь получает Premium доступ к функциям бота: возможность писать первым без взаимного лайка и просмотр кружков без стандартных ограничений сервиса.</p>
<h2>2. Стоимость и порядок оплаты</h2>
<p>Стоимость Premium доступа составляет {{.Price}} рублей. Оплата производится через платёжного провайдера. Доступ активируется после подтверждения успешной оплаты.</p>
