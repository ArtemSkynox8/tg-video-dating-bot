...
func initializeBot(ctx context.Context, cfg config.Config, webhook *dynamicWebhook, miniapp *dynamicWebhook) {
	...
	botService := services.NewDatingService(repo, maxClient, cfg.AdminPlatformIDs, cfg.PublicBaseURL, cfg.ReturnToBotURL, cfg.PremiumPrice, cfg.ContactInstructionVideoID, cfg.ContactInstructionVideoPath)
